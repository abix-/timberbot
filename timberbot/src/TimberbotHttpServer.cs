    ...
    class TimberbotHttpServer
    {
        ...
        private Dictionary<string, PostRouteDescriptor> BuildPostRoutes()
        {
            ...
            var routes = new[]
            {
                ...
                Queued("/api/agent/start", req => new LambdaWriteJob(req.Route, () => _service.Agent.Start(req.Body?.Value<string>("binary") ?? "claude", req.Body?.Value<string>("model"), req.Body?.Value<int>("timeout") ?? 120, req.Body?.Value<string>("goal")))),
