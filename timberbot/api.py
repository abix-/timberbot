...
class Timberbot:
    ...
    def _get_footprint(self, building):
        """Get the set of (x,y) tiles a building occupies based on origin, size, and orientation."""
        bx, by = building["x"], building["y"]
        # look up size from prefabs
        name = building["name"].replace("(Clone)", "")
        prefab = None
        ...
        if not prefab:
            return {(bx, by)}
        sx, sy = prefab.get("sizeX", 1), prefab.get("sizeY", 1)
        # orientation rotates the footprint
        # TODO: orientation handling is approximate -- may need tuning
        tiles = set()
        for dx in range(sx):
            for dy in range(sy):
    ...
    def adjacent(self, building_id):
        """Find all tiles adjacent to a building and whether they're free or occupied.

        Returns: {free: [(x,y,z)], occupied: [{x,y,name}]}

        Usage:
            bot.adjacent(-784782)  # check what's around the lumber mill
        """
        ...
        z = target.get("z", 2)
        footprint = self._get_footprint(target)

        # find all adjacent tiles (not in footprint)
        adj_tiles = set()
        ...
        # build occupancy map from all buildings
        occupied_map = {}
        for b in all_buildings:
            if "x" not in b or b["id"] == building_id:
                continue
            for (ox, oy) in self._get_footprint(b):
                occupied_map[(ox, oy)] = b["name"]

        free = []
        occupied = []
        for (ax, ay) in sorted(adj_tiles):
            if (ax, ay) in occupied_map:
                occupied.append({"x": ax, "y": ay, "name": occupied_map[(ax, ay)]})
            else:
                free.append({"x": ax, "y": ay, "z": z})
